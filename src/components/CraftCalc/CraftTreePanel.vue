    ...
    <div v-if="costData.total > 0" class="cost-card">
      ...
      <template v-else>
        ...
        <div v-for="row in costData.rows" :key="row.key" class="cost-pct-row">
          <div class="cost-pct-dot" :style="{background: row.color}" />
    ...
    <div class="tree-card">
      ...
      <div class="tree-body">
        <template v-for="(node, i) in result.tree" :key="i">
          <div v-if="node.divider" class="tree-divider" />
          <div v-else class="tree-node-row">
            <span class="tree-prefix">{{ node.prefix }}</span>
            <span class="tree-name" :class="node.cls">{{ node.name }}</span>
            <span v-if="node.tag" class="tree-tag" :class="'tag-' + node.tag">{{ t('tag.' + node.tagLabel) }}</span>
            <span
              v-if="node.cls !== 'tree-section' && node.amount"
              class="tree-amount"
              :class="node.cls"
